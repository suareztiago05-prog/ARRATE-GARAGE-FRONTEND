...
function MotoCard({ moto, esFavorita = false, onAlternarFavorito }) {
  ...
  return (
    <article className="moto-card">
      <div className="moto-card__image-container">
        <Link
          to={`/motos/${moto._id}`}
          className="moto-card__detail-link"
          aria-label={`Ver detalles de ${moto.nombre}`}
        >
          <img
            src={imagenPrincipal}
        ...
        {onAlternarFavorito && (
          <button
            type="button"
            className={`moto-card__favorite ${esFavorita ? "is-active" : ""}`}
            aria-label={esFavorita ? "Quitar de favoritos" : "Agregar a favoritos"}
            title={esFavorita ? "Quitar de favoritos" : "Agregar a favoritos"}
            onClick={() => onAlternarFavorito(moto)}
          >
            <i className={esFavorita ? "bi bi-heart-fill" : "bi bi-heart"}></i>
          </button>
        )}
      </div>
      ...
      <div className="moto-card__content">
        <div className="moto-card__header">
          <span className="moto-card__brand">{moto.marca?.nombre || "Marca"}</span>
          <span className="moto-card__category">{moto.categoria?.nombre || "Moto"}</span>
        </div>
        ...
        <div className="moto-card__footer">
          ...
          <Link to={`/motos/${moto._id}`} className="moto-card__button">
