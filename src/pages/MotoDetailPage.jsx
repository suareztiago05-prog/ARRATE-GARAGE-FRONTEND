...
function MotoDetailPage() {
  ...
  return <><Navbar/><main className="moto-detail"><div className="moto-detail__container">
    ...
    <div className="moto-detail__layout"><section className="moto-detail__gallery"><img src={imagenes[imagen]} alt={moto.nombre}/>{imagenes.length>1&&<div className="moto-detail__thumbs">{imagenes.map((url,index)=><button key={url} type="button" className={imagen===index?"active":""} onClick={()=>setImagen(index)}><img src={url} alt={`${moto.nombre} ${index+1}`}/></button>)}</div>}</section>
      <section className="moto-detail__info"><span className="moto-detail__brand">{moto.marca?.nombre}</span><h1>{moto.nombre}</h1><p className="moto-detail__category">{moto.categoria?.nombre}</p><strong className="moto-detail__price">{moto.precio?`USD ${Number(moto.precio).toLocaleString("es-AR")}`:"Consultar precio"}</strong>
        <div className="moto-detail__specs"><article><span>Cilindrada</span><strong>{moto.cilindrada} cc</strong></article><article><span>Stock</span><strong>{moto.stock} unidades</strong></article><article><span>Estado</span><strong>{moto.disponible!==false?"Disponible":"No disponible"}</strong></article></div>
